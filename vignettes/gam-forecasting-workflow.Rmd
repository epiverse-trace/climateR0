---
title: "Simple short-term forecasting with relative R0"
output: 
  html_document:
      toc: true
      toc_float: true
      code_folding: show
vignette: >
  %\VignetteIndexEntry{Simple short-term forecasting with relative R0}
...
```{r}
daily_dt <- weekly_to_daily_temps_function(fiji_2014)
...
daily_gi_samples_dt <- run_time_varying_monte_carlo(number_mc_samples = number_mc_samples,
           ...
           siraj_2017_params)
print(tail(daily_gi_samples_dt))
summary_daily_gi_samples_dt <- daily_gi_samples_dt[, list(MEDIAN = median(value),
    Q_0.025 = quantile(value, 0.025),
    Q_0.975 = quantile(value, 0.975)), by = c("TIME")]
ggplot(summary_daily_gi_samples_dt)+ geom_ribbon(aes(x = TIME, ymin = Q_0.025, ymax = Q_0.975), fill = "#8e0152", alpha = 0.5) +
  geom_line(aes(x = TIME, y = MEDIAN), col = "#276419") +
  labs(x = "Date", y = "Serial interval (days)") +
  theme_classic()
# Calculate SI for given date (relies on daily temperature data), SI then binned to weekly
max(daily_gi_samples_dt$TIME)
...
print(si_cases_dt)
# Function to weight past cases by estimated dengue serial interval
...
model_data <- si_cases_dt |> 
  ...
  dplyr::mutate(cumulative_cases = cumsum(cases), pop_susceptible = 1 - (cumulative_cases/pop),
         weighted_cases = weighted_lagged_cases,
...
horizon <- 2
prediction_dates <- seq(from = as.Date("2014-02-06"), to = as.Date("2014-09-01"), by = 7)
