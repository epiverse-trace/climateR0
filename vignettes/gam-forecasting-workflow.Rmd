...
model_data <- fiji_2014 |>
  ...
  dplyr::arrange(date) |>
  dplyr::mutate(
    cumulative_cases = cumsum(cases), pop_susceptible = 1 - (cumulative_cases / pop),
...
formatted_forecasts <- merge(
  all_forecasts |> filter(type == "forecast"), model_data |> dplyr::select(cases, date), by = "date", all = TRUE
  ) |>
  dplyr::mutate(
