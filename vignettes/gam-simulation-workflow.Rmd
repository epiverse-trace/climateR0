...
## Set up relative R0 and serial interval
...
### Forward prediction for specific time point and model
...
### Fit model for specific prediction date and run forward simulation
...
generate_prediction <- function(model_data,
  ...
  # DEBUG: model_data = model_data_2017; input_data = fiji_2017; model_in = model_2014
...
With the model and data in place, we generate N-week ahead forecasts for a given prediction date:
...
prediction_vals <- prediction_range[seq(4,20,2)]
...
for(ii in 1:length(prediction_vals)){
  ...
  generate_output <- generate_prediction(model_data,
  ...
  # Get intercept for R0 prediction
  r0_out <- exp(generate_output$model_out$coefficients[1])
  r0_estimate = data.frame(date = input_data$date,
                          r0_est = input_data$rR0*r0_out)
...
## Apply 2014 model to 2017 DENV-2 outbreak

As well as calibrating a model from scratch, we can use a model fitted to all the 2014 epidemic data for a future outbreak. First we use climate trends from 2014 to add temperature data and rR0 columns to a dataset for the 2017 DENV-2 outbreak in Fiji:

**NOTE: The below dataset is not yet properly loaded in the online package, so this code is in development**
```{r, eval=FALSE}
...
match_dates <- find_closest_days(fiji_2017$date,fiji_2014$date)

fiji_2017$av_temp <- fiji_2014$av_temp[match_dates]
fiji_2017$rR0 <- temperature_r0(fiji_2017$av_temp, "AeaeDENV")
```
Then we create the `model_data` object for this outbreak:
```{r, eval=FALSE}
model_data_2017 <- fiji_2017 |> 
  dplyr::mutate(date_numeric = as.numeric(date - min(date) + 1)) |> 
  dplyr::arrange(date) |> 
  dplyr::mutate(log_rR0 = log(rR0 + 0.0001),
         cumulative_cases = lag(cumsum(cases),1), 
...
# Set cumulative cases to zero initially
model_data_2017$cumulative_cases[1] <- 0
model_data_2017$pop_susceptible[1] <- 1
model_data_2017$log_pop_susceptible[1] <- log(1 + 0.0001)

# Note the below is constrained to match the temperature date range available
# It also includes a buffer of 5 weeks to account for serial interval calculation above
prediction_dates_2017 <- seq(from = (min(fiji_2017$date) + 5*7), 
                        to = max(fiji_2017$date), by = 7)
```

Now we run a forecast from an early prediction date (i.e. first available in 2017), using a later version of 2014 forecast model (i.e. calibrated to more data).
```{r, eval=FALSE}
prediction_start <- prediction_dates_2017[10]

model_2014 <- generate_output$model_out

generate_output <- generate_prediction(model_data_2017,
                                input_data = fiji_2017,
                                prediction_start,
                                horizon,
                                n_sim = 50,
                                model_in = model_2014 # Use model from final 2014 forecast
                                )
...
# Define training window
train_dataset <- model_data_2017[model_data_2017$date<=prediction_start,]
...
p1 <- ggplot(summary_stats,
      ...
      geom_point(data = model_data_2017,
                 aes(x = date,
                 y = cases,
                 color = "Data (all)"),
                 ) +
...
print(p1)
ggsave("combined_plot_D2_2017.pdf", plot = p1,width=4,height=2)
```

### Further options for developement

* Need better handling of uncertainty across multiple outbreaks, e.g. by using {brms} to make posterior of outbreak 1 the prior for outbreak 2.
