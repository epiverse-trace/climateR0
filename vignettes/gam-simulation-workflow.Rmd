...
generate_prediction <- function(model_data,
  ...
  # test_pred <- data.frame(log_rR0 = log(0.8), log_weighted_lagged_cases=1.61722, log_pop_susceptible=-0.25)
...
find_closest_days <- function(vec1,vec2){
  ...
  sapply(doy1, function(doy){
   closest_index <- which.min(abs(doy1 - doy))
   closest_index
  })
}
...
```{r}
prediction_start <- prediction_dates_2017[15]
