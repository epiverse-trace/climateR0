---
title: "Longer simulation-based forecasting with relative R0 (raw version with no functions)"
output: 
  html_document:
      toc: true
      toc_float: true
...
require(tidyr)
require(zoo)
load(here::here("data", "epidemicGrowth.RData"))
source(here::here("R", "serial_interval.R"))
...
prepare_data <- function(input_data, # Input data with date, cases and av_temp columns
  ...
  daily_dt <- weekly_to_daily_temps_function(input_data)
  #Times we can estimate SI
  fitting_dt <- fitting_data_table_function(daily_dt)
  # From Siraj et al 2017
  siraj_2017_params <- set_siraj_2017_params()
  daily_gi_samples_dt <- run_time_varying_monte_carlo(number_mc_samples = number_mc_samples,
           all_data = daily_dt,
           fitting_data = fitting_dt,
           siraj_2017_params)
  # Calculate SI for given date (relies on daily temperature data), SI then binned to weekly
  weekly_gi_samples_dt <- weekly_si_from_daily_si_function(daily_gi_samples_dt)
  fitting_dt <- subset(fitting_dt, EVALUATE == TRUE)
  fitting_dt <- data.table(fitting_dt)
# fitting_dt[which(cases == 0), cases:= 0]
  si_cases_dt <- weight_cases(weekly_gi_samples_dt, fitting_dt)

  # Add columns tracking recent lagged cases (for serial interval)
  # and cumulative cases (for estimating immunity)
  model_data <- si_cases_dt |> 
    ...
    dplyr::mutate(log_rR0 = log(rR0 + 0.0001),
           ...
           weighted_lagged_cases = weighted_lagged_cases,
...
generate_prediction <- function(model_data,
  ...
  if(is.null(model_in)){
    ...
    # Check: Covariate of log_weighted_lagged_cases is fixed or not
    if(fix_all_covariates==TRUE){
      gam_mod <- mgcv::gam(
      ...
      gam_mod <- mgcv::gam(
                  formula = cases ~ offset( 
                                    log_pop_susceptible) +
                                    log_rR0 +
                                    log_weighted_lagged_cases, #Should be covariate for log_rR0
                  family = nb(link = "log"),
                  data = train_data)
    }
  }else{
...
print(head(prepare_data_out))
# Define data for the model
model_data <- prepare_data_out$model_data
subsetted_input_data <- subset(input_data, date >= min(model_data$date) & date <= max(model_data$date))

# Define potential prediction date range
prediction_dates <- prepare_data_out$prediction_dates
prediction_dates
prediction_dates <- prediction_dates[which(prediction_dates >= min(subsetted_input_data$date) & prediction_dates <= max(subsetted_input_data$date))]
prediction_dates

# Specify range of potential forecast start dates
forecast_horizon <- 10
prediction_dates< - prediction_dates[forecast_horizon:(length(prediction_dates))]
prediction_dates <- prediction_dates[10:length(prediction_dates)]
...
prediction_vals                                          
# prediction_vals <- prediction_vals[forecast_horizon:length(prediction_vals)]

# prediction_dates< - prediction_dates[forecast_horizon:length(prediction_vals)]
```
...
for(ii in 1:length(prediction_dates)){
  ...
  generate_output <- generate_prediction(model_data, # Model formatted data
                                  ...
                                  fix_all_covariates = F # Constrain to be SIR-like 
                                  )
